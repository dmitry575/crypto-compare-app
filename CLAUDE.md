...
6. **Семантика цен: одни имена, два смысла.** `priceSell` и `priceBuy` означают
   **противоположное** на двух эндпоинтах, и это не опечатка бэкенда, а разные точки
   зрения.

   - **Разбивка по биржам** (`/v1/symbols/ticker/{ticker}`, поле `providerId`) —
     имена от лица биржи: `priceSell` = ask (биржа продаёт тебе, по ней пользователь
     покупает), `priceBuy` = bid. В UI подписи инвертированы.
   - **Best-выдача** (`/v1/symbols` — лента каталога, и `/v1/symbols/search`; поля
     `providerSellId` / `providerBuyId`) — имена от лица пользователя:
     `priceSell` = лучший bid, цена, по которой **продаёт он**; `priceBuy` = лучший
     ask, по которой покупает. Биржи в этих двух полях разные.

   Проверено на wire 2026-09-08: у `solusdt` search отдал `priceSell=103.35 @13` и
   `priceBuy=103.10 @19`, а разбивка — `max(priceBuy)=103.35 @13` и
   `min(priceSell)=103.10 @19`.

   **Спред один на всё приложение** и считается формулой бэкенда:
   `(sellPrice − buyPrice) / buyPrice * 100` — `spreadPercent()` в `core:helpers`.
   Знак значим и **в норме отрицателен**: купить дороже, чем продать, это обычное
   состояние рынка (на 2026-09-08 отрицательны 85% пар каталога, медиана −0.33%).
   Спред отдельной биржи пользователю не показывается; при паре с единственной биржей
   формула вырождается в него сама. Раньше каталог считал `(max − min) / min` по обоим
   полям сразу — это модуль, и 85% строк показывали знак наоборот.

   Серверный `spreadPercent` приходит только в `/v1/symbols/search`; в ленте каталога
   его нет, а в разбивке по биржам он есть, но равен нулю. Поэтому считаем сами —
   формула совпадает с серверной до последнего знака.
