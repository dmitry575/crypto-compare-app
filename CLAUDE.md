...
одним запросом на всю глубину (`GET /v1/klines/{providerId}`) и живёт в памяти.
...
4. **График — по выбранной бирже, история одним запросом.**
   Свечи берутся с нашего бэкенда по `providerId` (`GET /v1/klines/{providerId}`),
   поэтому график привязан к выбранной бирже и **перезагружается** при её смене в
   `ExchangeSelector`. За раз берём `HISTORY_LIMIT` свечей (потолок бэкенда — 1000)
   и держим их в памяти ViewModel по ключу `providerId+timeframe`; кеша на диске нет.
   Немаркетные периоды не растягивают ось: свечи рисуются по индексу, а не по времени.

   **Постраничной догрузки на ходу нет — и это решение, а не недоделка.** vico меряет
   скролл в пикселях от начала содержимого: дописанная слева страница сдвигает начало,
   и кадр уезжает на её ширину. Компенсировать нечем — публичный `scrollState.scroll()`
   ждёт `isScrollInProgress` и применяется через кадр-другой после того, как новые данные
   уже нарисованы, так что прыжок видно всегда. Пока ряд неизменен, прыгать нечему.
   Если глубины перестанет хватать, правильный ход — не возвращать догрузку «как было»,
   а зафиксировать координатную сетку: `rangeProvider.getMinX` на всю будущую глубину,
   `x = -offset` свечи (возраст в единицах бэкенда), страницы дорисовываются внутри уже
   существующего пространства — тогда начало содержимого не двигается.

   Ось Y считается по **видимому кадру**, иначе одна свеча из глубины (памп на 10x)
   сплющивает текущие в ленту. Границы кадра отдаёт сам vico через
   `VisibleXRangeItemPlacer` (обёртка над `HorizontalAxis.ItemPlacer`): считать их в
   композиции по `scrollState` нельзя — скролл меряется по границам слоя, а не по ширине
   композиции, и оценка тем хуже, чем дальше уехал кадр. Именно из-за этой ошибки свечи
   уходили за границы оси и график выглядел пустым. Кадр обновляется только на заметный
   сдвиг (`FRAME_UPDATE_RATIO`): ширина оси цен зависит от подписей, подписи — от кадра,
   и без порога эта петля может зациклиться.

   Модель собирается напрямую (`CartesianChartModel`), без `CartesianChartModelProducer`:
   продюсер применяет данные асинхронной транзакцией и только в ней пересчитывает
   диапазоны осей, поэтому ось Y отставала от кадра.

   **Границы зума обязательно `remember`.** `Zoom.x(...)` — фабрика, каждый вызов даёт
   новый объект, а `rememberVicoZoomState` держит `initialZoom/minZoom/maxZoom` ключами
   `rememberSaveable`: незапомненные границы пересоздают состояние зума на каждой
   рекомпозиции (живой бар рекомпозит график дважды в секунду), и зум откатывается к
   начальному — со стороны это выглядит как «зум не работает». Та же ловушка у
   `rememberVicoScrollState` с дефолтным `spring()`.
