...
- `targetSdk` поднят до 37 отдельным коммитом перед релизом 1.2, а не заодно с
  обновлением зависимостей: подъём меняет рантайм-поведение. Следующий подъём —
  так же, отдельно и с проверкой на устройстве с этой версией Android.
...
- Ошибки не пробрасываются как исключения в UI: репозитории заворачивают вызовы в
  `appRunCatching(crashReporter)`, и наружу уходит `AppError` (`core:model`) — что
  случилось, без типов Retrofit, Room и Firebase. Текст на языке интерфейса
  подбирает только UI (`AppError.message()` в `core:ui`). Непредвиденное — ошибка
  бэкенда, базы, неопознанное исключение — уходит в Crashlytics; сеть и вход нет.
...
1. **Каталог тикеров — Paging 3, а не список в памяти.**
   Источник истины — Room. `SymbolDao.pagingPairs()` собирает строки каталога прямо в SQL
   (поиск, фильтр избранного и направления — тоже в запросе).
   ...
   **Строка — символ, а не тикер.** Бэкенд делит пару на символы по сетям: USDC в
   Ethereum и USDC в Solana — разные активы, и сравнение их цен давало бы спреды в
   тысячи процентов. Поэтому символы тикера не сводятся ни в каталоге, ни на деталях,
   ни в сравнении: экран пары открывается по `symbolId`, биржи и лучшая пара берутся
   только этого символа, живые события чужих символов тикера отсекаются (подписка в
   сокете по тикеру, и они приходят вперемешку). Строка тикера с несколькими символами
   помечается сетями. Сети приходят полем `network` в формате источника — `eth`,
   `Ethereum`, `ERC20`, `ETH-ERC20` это одна сеть — и сводятся к одному виду в
   `networkNames` (`core:helpers`); словарь — `NetworkConstants`.

   **Избранное — тоже по символу.** Ключ и в Room (`favourite_symbols`), и в
   документе Firestore — `symbolId`, тикер лежит рядом денормализованным, чтобы
   избранное было чем назвать, когда символа в каталоге уже нет. Пока ключом был
   тикер, звезда на одной сети зажигалась на всех строках этой пары. Документы
   старого формата (id = тикер, поля `symbolId` нет) синхронизация разворачивает
   во все символы тикера и удаляет — но только когда каталог уже загружен, иначе
   разворачивать не во что и документ остаётся до следующего раза.

2. **Цены из WebSocket пишутся батчами, и пишет их один писатель.**
   Тики копятся в `Map<symbolId, TickerBestPrice>` и раз в
   `WebSocketConstants.PRICE_FLUSH_INTERVAL_MS` уходят в БД одним заходом; Room сам
   инвалидирует видимые страницы. Обновлять UI на каждый тик нельзя — это давало
   непрерывный GC и пропуск кадров.

   Пишет лучшие пары `SyncLiveBestPricesUseCase`, один на процесс: его запускает
   `CryptoCompareApp` в `ProcessLifecycleOwner.lifecycleScope`. Экраны каталог не
   пишут — только читают. Раньше пачку вела каждая ViewModel, и портфель, открытый
   поверх живого каталога, писал те же тики второй раз. Цены бирж покупки
   (`portfolio_quotes`) пишет портфель: они нужны только ему и только для его позиций.
...
6. **Семантика цен: одни имена, два смысла.** `priceSell` и `priceBuy` означают
   ...
   **Спред приходит с бэкенда готовым, приложение его не считает.** Он есть и в ленте
   каталога, и в best-выдаче, и в разбивке по биржам (там — bid/ask одной площадки).
   На бэкенде же отсеиваются протухшие котировки: быть неправым в выгодную сторону —
   ровно то, что выигрывает сравнение, поэтому до правки одна биржа с зависшей ценой
   рисовала `+90.9%` арбитража. Пороги живут в `AppSettings` бэкенда
   (`BestPriceMaxQuoteAgeSeconds`, `BestPriceMaxDeviationPercent`, `MaxSpreadPercent`).
   `spreadPercent()` в `core:helpers` остаётся для расчёта по разбивке — там, где
   строку каталога взять неоткуда.

   **В сокете два события про цену, и путать их нельзя.** Тип 4
   (`SymbolPriceChange`) — котировка одной биржи, у него `providerId`, и живут на нём
   детальный экран и сравнение. Тип 5 (`SymbolBestPriceChange`) — лучшая пара по
   тикеру, той же формы, что строка каталога, и **только он** пишется в таблицу
   `symbols`. У типа 4 `symbolId` общий на весь тикер, поэтому каждая биржа затирала
   бы строку своими ценами, и каталог показывал бы bid/ask последней тикнувшей
   площадки вместо разницы между биржами. Так и было до #19.
...
8. **Один `NavHostController` на приложение.**
   ...
   **Нижняя панель — две вкладки, «Пары» и «Портфель»,** и видна только на их корнях.
   Вкладка переключается с `popUpTo(navController.graph.id) { saveState = true }` +
   `restoreState`: якорь — корневой граф, а не граф каталога. Каталог после заставки
   сам лежит в корне стека, и прыжок к нему сохранял бы и тут же перетирал состояние
   обеих вкладок — вернувшись, пользователь терял бы фильтры и прокрутку. Под
   вкладкой портфеля в стеке ничего нет, поэтому «назад» с её корня перехвачен и ведёт
   в каталог, а не закрывает приложение.

9. **Поток котировок живёт, только пока приложение на экране.**
   `TickerStreamLifecycleObserver` висит на `ProcessLifecycleOwner` и в фоне зовёт
   `pause()`, а не `disconnect()`: набор подписок остаётся, и `resume()` возвращает его
   как было. `disconnect()` означает «соединение больше не нужно», и после него
   `resume()` ничего не открывает.

   **Пропущенные тики сервер не досылает**, и снимка цены при подписке тоже нет:
   первое событие приходит, когда тикнет биржа, — у `athbtc` за 20 секунд после
   подписки их было два (проверено 2026-09-15; подписка — тип 1, тип 2 — отписка). Поэтому после каждого нового открытия соединения
   (`reconnects`) экраны берут цены через REST: каталог — лучшие пары видимых тикеров,
   детали и сравнение — свои данные целиком. Без этого после возврата из фона строка
   малоликвидной пары показывала бы цену часовой давности до следующего события.

   Колбэки OkHttp сверяются с номером текущего сокета. Закрытый сокет присылает
   `onClosed` позже, чем открывается новый, и раньше этот поздний колбэк ставил
   `Disconnected` поверх живого соединения и открывал третий сокет, пока второй
   висел без ссылки и дублировал тики.

10. **Вход необязателен: каталог, детали и сравнение работают без аккаунта.**
    Заставка проверяет только флаг онбординга и всегда уходит в каталог —
    раньше она разводила на каталог и экран входа, и первым, что видел
    установивший приложение, была форма регистрации.

    Аккаунт нужен ровно одному — избранному: оно хранится в Firestore и тем и
    ценно, что общее на всех устройствах. Поэтому звезда и фильтр «Избранное»
    у гостя показывают приглашение войти (`signInRequired` → снекбар с
    действием), а профиль вместо почты и раздела «Аккаунт» — карточку входа;
    тема, язык и политика доступны всем.

    Гостя определяем через `GetCurrentUserUseCase`, а не через флаг в UI-состоянии:
    поток `ObserveAuthStateUseCase` приезжает асинхронно, и звезда, нажатая сразу
    после запуска, у вошедшего пользователя просила бы вход. Поток остаётся для
    обратного случая — после выхода фильтр «Избранное» гаснет сам, иначе каталог
    остался бы пустым списком без объяснения.

    Экран входа открывается **поверх** каталога или профиля и после успеха
    закрывается целиком (`popBackStack` по графу авторизации), возвращая туда,
    откуда его позвали.

11. **У каждого кеша есть предел и ответ на «а если он старый».**
    Сводка — `core/data/CACHE_POLICY.md`: что где лежит, сколько живёт, чем
    ограничено и что видит пользователь, когда данные устарели. Новый кеш без
    строки в этой таблице — ошибка. Самый неочевидный предел — свечи: экран пары
    держит последние `MAX_CACHED_CHARTS` графиков (биржа × масштаб), давно не
    открытые вытесняются через `putRecent`, иначе перебор бирж копил бы их все.

    Фоновая перекачка каталога ставится с `UPDATE`, а не `KEEP`: при `KEEP`
    смена интервала не доходила до уже стоящей работы, и у установок до
    2026-08-26 каталог перекачивался раз в сутки даже после обновления.
