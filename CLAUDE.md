...
11. **У каждого кеша есть предел и ответ на «а если он старый».**
    ...
    Фоновая перекачка каталога ставится с `KEEP`, поэтому смена интервала не
    доходит до уже стоящей работы: у установок до 2026-08-26 каталог
    перекачивается раз в сутки. Так и оставлено; что изменил бы `UPDATE`,
    описано там же.
