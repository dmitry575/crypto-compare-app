 ...
 * открывается с `Zoom.x(visibleCount)`, а после пинч-зума vico сам отдаёт
 * актуальное число видимых свечей через `zoomState.value`. Из пикселей берём
 * только долю прокрутки — от ширины полосы оси Y она не зависит, так что
 * обратной связи «шире подписи → уже кадр → другое окно → другие подписи»
 * не возникает.
 *
