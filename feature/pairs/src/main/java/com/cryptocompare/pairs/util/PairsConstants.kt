...
object PairsConstants {
    ...
    object Chart {
        ...
        /**
         * За сколько свечей до старого края начинаем подгружать следующую страницу:
         * запас, чтобы пользователь не упёрся в пустоту, пока страница едет.
         */
        const val LOAD_MORE_MARGIN = 10
        ...
        /**
         * Подписи оси X — тирами, как на биржах: внутри дня показываем время, на
         * смене дня — дату, на смене года — дату с годом. Плюс постоянная дата
         * левого края кадра ([LABEL_DATE_PATTERN]) в углу графика, чтобы день и год
         * были видны всегда, даже когда весь кадр внутри одного дня. Форматы
         * компактные и не зависят от локали (без названий месяцев).
         */
        const val LABEL_DATE_PATTERN = "dd.MM.yy"
        const val LABEL_DAY_PATTERN = "dd.MM"
        const val LABEL_TIME_PATTERN = "HH:mm"

        /** Ключ сравнения свечей по дню/году (год — первые 4 символа) при выборе тира. */
        const val LABEL_DAY_KEY_PATTERN = "yyyyMMdd"

        /** Отступ у постоянной подписи даты в углу графика. */
        val axisDateLabelPadding: Dp = 4.dp
